...
def add_exact_workdays_constraints(
    ...
    各従業員の勤務日数が指定値の±max_diff日以内になるように制約を追加する。

    さらに、指定値からのズレを表す employee_workdays_diff を返す。
    この diff は main.py 側で目的関数に使う。
    ...
    for e in employees:
        ...
        model.Add(employee_workdays[e] == actual_workdays)
